...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
     ...
     //   sh "/usr/local/bin/helm upgrade --dry-run --debug --install ${args.name} ${args.chart_dir} --set image.repository=stockpile-baladockerreg1.jfrog.io/bakuppus/amazonlinux-httpd --set image.tag=${args.tag} --set imagePullSecrets.secret=jfrog-seshang-credentials-secret,Replicas=${args.replicas},Cpu=${args.cpu},Memory=${args.memory},DomainName=${args.name} --namespace=${args.name}"
        sh "/usr/local/bin/helm upgrade --dry-run --debug --install ${args.name} ${args.chart_dir} --set image.repository=stockpile-baladockerreg1.jfrog.io/bakuppus/amazonlinux-httpd --set image.tag=${args.tag},Replicas=${args.replicas},Cpu=${args.cpu},Memory=${args.memory},DomainName=${args.name} --namespace=${args.name}"
    } else {
        println "Running deployment"
        sh "/usr/local/bin/helm upgrade --install ${args.name} ${args.chart_dir} --set image.repository=stockpile-baladockerreg1.jfrog.io/bakuppus/amazonlinux-httpd --set image.tag=${args.tag},Replicas=${args.replicas},Cpu=${args.cpu},Memory=${args.memory},DomainName=${args.name} --namespace=${args.name}"
    ...
    docker.withRegistry("${registry_url}", "${docker_creds_id}") {
        ...
        try {
            ...
            docker.image("${maintainer_name}/${container_name}:${build_tag}").withRun("--name=${container_name}  -p 80:80 ")  { c ->
                ...
                //
                waitUntil {
                    sh "ss -antup | grep -w 80 | grep LISTEN | wc -l | tr -d '\n' > /tmp/wait_results"
