...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        sh "/usr/local/bin/helm upgrade --dry-run --debug --install ${args.name} ${args.chart_dir} --set image.repository=stockpile-baladockerreg1.jfrog.io/bakuppus/amazonlinux-httpd --set image.tag=${args.tag} --set imagePullSecrets.secret=jfrog-seshang-credentials,Replicas=${args.replicas},Cpu=${args.cpu},Memory=${args.memory},DomainName=${args.name} --namespace=${args.name}"
    } else {
